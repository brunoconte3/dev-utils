...
use DevUtils\Exception\CoverageGateException;
...
final class CoverageGate
{
    ...
    {
        if (!is_numeric($value)) {
            throw new CoverageGateException("Threshold must be numeric, received '$value'.");
        }
        return (float) $value;
    }
    ...
    public static function readRatio(string $reportFile): float
    {
        if (!is_file($reportFile) || !is_readable($reportFile)) {
            throw new CoverageGateException("Coverage report not found: '$reportFile'.");
        }

        $xml = self::loadXml($reportFile);
        if (!$xml instanceof SimpleXMLElement) {
            throw new CoverageGateException("Coverage report is not valid XML: '$reportFile'.");
        }

        if (!isset($xml->project->directory->totals->lines['percent'])) {
            throw new CoverageGateException("Coverage report has no line totals: '$reportFile'.");
        }
