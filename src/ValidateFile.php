...
class ValidateFile
{
    ...
    public static function validateMaxUploadSize(
        ...
        return self::validateFileSize($file, $rule, $message, fn($size, $rule) => $size > $rule, 'máximo');
    }
    ...
    public static function validateMinUploadSize(
        ...
        return self::validateFileSize($file, $rule, $message, fn($size, $rule) => $size < $rule, 'mínimo');
    }
    ...
    public static function validateFileUploadMandatory(
        ...
        $hasNoFileError = (
            ...
            isset($file['error']) &&
            (
                (is_array($file['error']) && isset($file['error'][0]) && $file['error'][0] === UPLOAD_ERR_NO_FILE) ||
                (!is_array($file['error']) && $file['error'] === UPLOAD_ERR_NO_FILE)
            )
        );

        return $hasNoFileError ? [$message] : [];
    }
    ...
    public static function validateMaximumFileNumbers(
        ...
        $message = $message ?: "O campo {$field} deve conter, no máximo {$rule} arquivo(s)!";
        return self::validateFileCount($file) > $rule ? [$message] : [];
    }
    ...
    public static function validateMinimumFileNumbers(
        ...
        $message = $message ?: "O campo {$field} deve conter, no mínimo {$rule} arquivo(s)!";
        return self::validateFileCount($file) < $rule ? [$message] : [];
    }
}
